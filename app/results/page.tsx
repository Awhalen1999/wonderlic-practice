...
type CategoryBreakdown = Record<QuestionCategory, { correct: number; total: number }>
...
export default function ResultsPage() {
  ...
  const categoryBreakdown = lastResult?.categoryBreakdown as CategoryBreakdown | undefined
        ...
        <div className="bg-white border border-zinc-200 rounded-2xl p-8 text-center">
          ...
          <div className="bg-zinc-50 rounded-xl p-4 mb-6 text-sm text-zinc-600">
            {score >= 35 ? (
              ...
              <>💡 Room to grow. Dig into practice mode and those explanations — you&apos;ve got this.</>
            )}
          </div>
        ...
        {categoryBreakdown && (
          <div className="bg-white border border-zinc-200 rounded-2xl p-6">
            ...
            <div className="space-y-3">
              {(Object.entries(categoryBreakdown) as [QuestionCategory, { correct: number; total: number }][])
                .filter(([, v]) => v.total > 0)
                      ...
                      <div className="w-full bg-zinc-100 rounded-full h-1.5">
                        <div
                          className={`h-1.5 rounded-full transition-all ${
                            catPct >= 70 ? 'bg-emerald-500' : catPct >= 50 ? 'bg-indigo-500' : 'bg-amber-500'
                          }`}
                          style={{ width: `${catPct}%` }}
                        />
                      </div>
